...
There is no test runner configured — `npm run build` and `npm run lint` are the verification steps. To go live against Supabase: copy `.env.example` to `.env.local`, fill both vars, run `supabase/schema.sql` in the project's SQL editor, then `supabase/seed_admin.sql` (edited with your email/password) to create the single Admin account.
...
- **Auth (dual-mode)**: `src/lib/auth.ts` picks per `isSupabaseConfigured` — Supabase mode uses `supabase.auth` (signUp carries name/state/phone/nin/document path as user metadata, mirrored to `profiles` + `identity_documents` by a DB trigger; email-confirmation flows are handled on the signup form); local mode stores self-registered accounts in localStorage with SHA-256-hashed passwords. Both modes expose the session through one subscribe/snapshot pair consumed by `AuthProvider` via `useSyncExternalStore` (cross-tab sync included). `AuthGuard` redirects signed-out visitors to `/login?next=…`. Login/signup share the `AuthScreen` frame and `fieldStyles.ts` input classes.
- **Two roles, one seeded Admin**: `UserRole` is `"Admin" | "Member"` (`src/lib/roles.ts`). Anyone can register on `/signup` and always becomes a **Member** — the signup trigger hard-assigns the role (client metadata is never trusted). The single **Admin** account is created by running `supabase/seed_admin.sql` once in the SQL editor (inserts the `auth.users` row + promotes the profile) — **no UI path, ever**.
- **Registration approval (Supabase mode)**: new signups collect an 11-digit NIN plus one government-ID photo (compressed client-side via `src/lib/images.ts`) and are held `pending` until the Admin approves them from the Pending Approvals tab in the Admin Console — login is blocked until approved. Local demo mode auto-approves signups (no backend → no admin to review). The approvals tab is a documented exception to "components don't fetch": `src/lib/approvals.ts` runs in the browser because identity documents (PII in `identity_documents` + a private `identity-documents` storage bucket) are Admin-gated by RLS and must never ride the anon server path. Approve/reject goes through the `review_user()` RPC; existing DBs migrate via `supabase/migrations/20260730_single_admin.sql`.
- **Admin content editing**: the Admin Console also has Alerts (full CRUD), State Risks (edit-only) and Data Sources (full CRUD) tabs, writing from the browser through `src/lib/adminContent.ts` (same documented exception as approvals; enforced by `is_admin()` RLS policies). Officers acknowledge alerts via the `acknowledge_alert()` RPC — direct alert writes are Admin-only. The Data Sources tab's "Refresh live data" button POSTs to `/api/admin/refresh-data`, which verifies the caller is the approved Admin (Bearer token → profile check) and then runs the WHO/HDX ingest (`scripts/ingest/*`) with `SUPABASE_SERVICE_ROLE_KEY`. Pipeline-only tables (trends, forecasts, LGAs, epi reports) have no hand-edit UI and no browser write policies.
